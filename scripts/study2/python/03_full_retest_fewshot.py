#!/usr/bin/env python3
"""
03_full_retest_fewshot.py
Purpose: Full retest of few-shot labeling on all 40 participants to verify 50-50 split stability
...
def main():
    """Main execution function for full retest"""
    print("🔄 FULL RETEST: Few-Shot LLM Labeling on All 40 Participants")
    print("=" * 70)
    print(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("Purpose: Verify 50-50 split stability from original labeling")
    print("=" * 70)
    
    # Load participant data
    df = pd.read_csv('results/study2/clean_participant_dataset_fewshot.csv')
    print(f"\n📊 Loaded {len(df)} participants from clean dataset")
    
    # Show original distribution
    original_counts = df['ai_label'].value_counts()
    print(f"\nOriginal Distribution:")
    print(f"  INTERESTED: {original_counts.get('INTERESTED', 0)}")
    print(f"  NOT_INTERESTED: {original_counts.get('NOT_INTERESTED', 0)}")
    
    # Setup model
    model = setup_gemini()
    
    # Process all participants
    print("\n🚀 Starting retest labeling...")
    results = []
    
    for idx, row in df.iterrows():
        participant_id = row['participant_id']
        combined_text = row['combined_text']
        original_label = row['ai_label']
        
        print(f"\nParticipant {participant_id} (Original: {original_label})...", end=' ')
        
        # Get new label
        new_label = label_participant_fewshot(model, combined_text)
        print(f"Retest: {new_label}", end='')
    ...
    # Save results
    results_df.to_csv('results/study2/full_retest_results.csv', index=False)
    ...
    # New distribution
    retest_counts = results_df['retest_label'].value_counts()
    print(f"\nRetest Distribution:")
    print(f"  INTERESTED: {retest_counts.get('INTERESTED', 0)}")
    print(f"  NOT_INTERESTED: {retest_counts.get('NOT_INTERESTED', 0)}")
    print(f"  ERROR: {retest_counts.get('ERROR', 0)}")
    
    # Changes
    n_changed = results_df['changed'].sum()
    pct_changed = (n_changed / len(results_df)) * 100
    print(f"\nLabel Changes:")
    print(f"  Changed: {n_changed}/{len(results_df)} ({pct_changed:.1f}%)")
    print(f"  Consistent: {len(results_df) - n_changed}/{len(results_df)} ({100-pct_changed:.1f}%)")
    
    # Show which participants changed
    if n_changed > 0:
        print(f"\nParticipants that changed labels:")
        changed_df = results_df[results_df['changed']]
        for _, row in changed_df.iterrows():
            print(f"  {row['participant_id']}: {row['original_label']} → {row['retest_label']}")
    ...
    original_numeric = [label_map.get(label, -1) for label in results_df['original_label']]
    retest_numeric = [label_map.get(label, -1) for label in results_df['retest_label']]
    
    # Filter out errors
    valid_pairs = [(o, r) for o, r in zip(original_numeric, retest_numeric) if o != -1 and r != -1]
    if valid_pairs:
        original_valid = [p[0] for p in valid_pairs]
        retest_valid = [p[1] for p in valid_pairs]
        kappa = cohen_kappa_score(original_valid, retest_valid)
    ...
    print("\n✅ Full retest complete!")
    print("Results saved to: results/study2/full_retest_results.csv")
