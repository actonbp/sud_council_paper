#!/usr/bin/env python3
"""
01_label_interest_gemini.py
Purpose: Use Google Gemini 2.5 Pro to label focus group PARTICIPANTS for SUD counseling interest
...
def create_labeling_prompt():
    """Create the prompt for Gemini to label participant interest level"""
    return """
...
- "INTERESTED" - Participant expresses positive interest, curiosity, or attraction to SUD counseling careers
- "NOT_INTERESTED" - Participant expresses disinterest, concerns, or negative views about SUD counseling careers
...
- Consider the participant's overall stance across all their utterances
- If mixed signals, lean toward the more definitive statements about personal career interest
...
def label_participant(model_or_client, participant_text, max_retries=3):
    """Label a participant's overall interest using Gemini"""
    prompt = create_labeling_prompt() + f'"{participant_text}"'
...
def main():
    ...
    print("🚀 Starting Gemini participant-level labeling...")
    ...
    cache_file = Path('results/study2/participant_labels.csv')
    ...
    for idx, row in participants_to_process.iterrows():
        ...
        label = label_participant(model, combined_text)
    ...
    if results:
        ...
        print("\n📊 Participant Labeling Summary:")
        for label, count in label_counts.items():
            pct = (count / len(final_df)) * 100
            print(f"  {label}: {count} ({pct:.1f}%)")
        
        print(f"\n✅ Labeling complete! {len(final_df)} total participants labeled")
    else:
