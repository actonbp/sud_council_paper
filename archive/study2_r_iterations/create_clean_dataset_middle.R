#!/usr/bin/env Rscript
# Create clean dataset with participant utterances and MIDDLE GROUND AI labels
...
# Load MIDDLE GROUND AI labels
middle_labels <- read_csv("results/study2/participant_labels_middle.csv", show_col_types = FALSE) %>%
  filter(gemini_label %in% c("INTERESTED", "NOT_INTERESTED")) %>%
  distinct(participant_id, .keep_all = TRUE) %>%
  mutate(participant_id = as.character(participant_id))

# Merge utterances with middle ground labels
final_dataset <- participant_combined %>%
  left_join(middle_labels, by = "participant_id") %>%
...
# Save clean dataset
write_csv(final_dataset, "results/study2/clean_participant_dataset_middle.csv")

# Print summary
cat("📊 Clean Dataset Summary (MIDDLE GROUND):\n")
cat("Total participants:", nrow(final_dataset), "\n")
cat("AI Labels:\n")
print(table(final_dataset$ai_label))
cat("\nDataset saved to: results/study2/clean_participant_dataset_middle.csv\n")