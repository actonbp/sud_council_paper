#!/usr/bin/env python3
"""
01_label_interest_middle.py
Purpose: Use Google Gemini 1.5 Pro to label focus group PARTICIPANTS for SUD counseling interest (MIDDLE GROUND)
...
def create_middle_labeling_prompt():
    """Create the MIDDLE GROUND prompt for Gemini to label participant interest level"""
    return """
...
- "INTERESTED" - Participant shows personal interest, curiosity, or positive consideration of SUD counseling careers
- "NOT_INTERESTED" - Participant shows disinterest, negative views, or lacks personal engagement with SUD counseling careers

MIDDLE GROUND GUIDELINES - Slightly more inclusive than strict, but not too liberal:
- INTERESTED includes: expressing personal curiosity, asking thoughtful questions, showing empathy for people with substance issues, mentioning they could "see themselves" doing it, discussing it as a genuine possibility, expressing interest despite some concerns, showing genuine engagement beyond just answering questions
- Also INTERESTED: expressing passion for helping people with addictions, being undecided but showing genuine consideration, asking follow-up questions about the work

- NOT_INTERESTED includes: purely academic discussion, focusing mainly on negatives without interest, casual mentions without personal connection, clear statements it's not for them, discussing only barriers without expressing curiosity

Key principle: Look for genuine personal curiosity and consideration, not just polite participation. Someone can have concerns but still be interested if they show authentic engagement with the possibility.

If someone shows mixed signals, lean slightly toward INTERESTED if there's any genuine personal consideration or curiosity.
...
def label_participant_middle(model_or_client, participant_text, max_retries=3):
    """Label a participant's overall interest using middle ground Gemini prompt"""
    prompt = create_middle_labeling_prompt() + f'"{participant_text}"'
...
def main():
    ...
    print("🚀 Starting Gemini participant-level labeling (MIDDLE GROUND)...")
    ...
    cache_file = Path('results/study2/participant_labels_middle.csv')
    ...
    for idx, row in participants_to_process.iterrows():
        ...
        label = label_participant_middle(model, combined_text)
    ...
    if results:
        ...
        print("\n📊 Participant Labeling Summary (MIDDLE GROUND):")
        for label, count in label_counts.items():
            pct = (count / len(final_df)) * 100
            print(f"  {label}: {count} ({pct:.1f}%)")
        
        print(f"\n✅ Middle ground labeling complete! {len(final_df)} total participants labeled")
    else:
