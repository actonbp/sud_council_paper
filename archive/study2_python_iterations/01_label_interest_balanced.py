#!/usr/bin/env python3
"""
01_label_interest_balanced.py
Purpose: Use Google Gemini 1.5 Pro to label focus group PARTICIPANTS for SUD counseling interest (BALANCED VERSION)
...
def create_balanced_labeling_prompt():
    """Create the BALANCED prompt for Gemini to label participant interest level"""
    return """
...
- "INTERESTED" - Participant shows interest, openness, or positive engagement with SUD counseling careers
- "NOT_INTERESTED" - Participant shows disinterest, negative views, or lack of engagement with SUD counseling careers

BALANCED GUIDELINES - Be generous in interpreting interest:
- INTERESTED includes: expressing curiosity, asking questions about the field, showing empathy for people with substance issues, discussing potential benefits of the work, expressing any personal connection to helping people, mentioning they "could see themselves" doing it, discussing it as a possibility, showing engagement with the topic beyond just answering questions
- INTERESTED also includes: discussing challenges but still seeming engaged, expressing mixed feelings but showing some positive aspects, being undecided but leaning toward consideration
- NOT_INTERESTED includes: expressing clear disinterest, focusing primarily on negatives (low pay, difficulty, etc.) without positive engagement, stating it's "not for them," showing no personal connection or curiosity, purely academic discussion without personal investment

Key principle: If someone is actively engaging with the idea of SUD counseling as a career possibility (even with reservations), lean toward INTERESTED. Only choose NOT_INTERESTED if they show clear disengagement or negative positioning.

Look for engagement and consideration, not just explicit enthusiasm.
...
def label_participant_balanced(model_or_client, participant_text, max_retries=3):
    """Label a participant's overall interest using balanced Gemini prompt"""
    prompt = create_balanced_labeling_prompt() + f'"{participant_text}"'
...
def main():
    ...
    print("🚀 Starting Gemini participant-level labeling (BALANCED VERSION)...")
    ...
    cache_file = Path('results/study2/participant_labels_balanced.csv')
    ...
    for idx, row in participants_to_process.iterrows():
        ...
        label = label_participant_balanced(model, combined_text)
    ...
    if results:
        ...
        print("\n📊 Participant Labeling Summary (BALANCED):")
        for label, count in label_counts.items():
            pct = (count / len(final_df)) * 100
            print(f"  {label}: {count} ({pct:.1f}%)")
        
        print(f"\n✅ Balanced labeling complete! {len(final_df)} total participants labeled")
    else:
